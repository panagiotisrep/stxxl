...
STXXL_BEGIN_NAMESPACE
  namespace boa_opt
  {
    ...
    public:
      ...
      {
        size_t internal_memory = (m_tiers.size() * PageSize * Pages * BlockSize)
          + (m_tiers.size() * PageSize * Pages * BlockSize);
      ...
      size_t get_size_bytes() const
      {
        ...
        {
          runs_size += tier.m_run.size();
        }

        return m_elements_in_external_memory * sizeof(data_element)
          + runs_size * sizeof(run_element)
          + m_tiers[0].m_routing_filter->get_size_bytes();
      }
    ...
    private:
      ...
      std::atomic<int> m_mergings;

      struct tier
      {
        std::unique_ptr<std::mutex> m_merging;
        std::unique_ptr<std::condition_variable> m_ready_cv;
        external_vector m_run;
        std::unique_ptr<routing_filter<HashType, 2, PageSize, BlockSize>> m_routing_filter;
        ...
        tier()
        {
          m_merging.reset(new std::mutex());
          m_ready_cv.reset(new std::condition_variable());
        }
      };
      ...
      size_t get_vector_size_for_tier(unsigned int tier) const
      {
        return m_tiers[tier].m_runs[0].m_run_size * RunsPerTier;
      }

      size_t get_run_offset(unsigned int tier, unsigned int run) const
      {
        return m_tiers[tier].vector_run_offset(run);
      }
    ...
    public:
      ...
      {
        while (m_mergings.load(std::memory_order_acquire) != 0) {
          std::this_thread::yield(); // or pause instruction on x86
          std::cout << "still merging" << std::endl;
        }
    ...
    private:
      routing_filter<HashType, 2, PageSize, BlockSize>* create_routing_filter(
        uint32_t capacity, uint16_t tier_level = 0)
      {
        ...
        // auto l_hl = p
        return new routing_filter<HashType, 2, PageSize, BlockSize>(run_size);
      }
      ...
      {
        {
          std::lock_guard<std::mutex> lock(*(m_tiers[0].m_merging));

          // if (*(m_tiers[0].m_merging))
          // {
          //
          // }
          ...
          if (m_tiers[0].m_run.size() < get_vector_size_for_tier(0))
          {
            m_tiers[0].m_run.resize(get_vector_size_for_tier(0));
          }
    ...
    private:
      ...
      void compact_tiers()
      {
        ...
        for (int tier_no = 0; tier_no < m_tiers.size(); ++tier_no)
        {
          if (active_runs(m_tiers[tier_no]) >= RunsPerTier)
          {
            ...
            int first_inactive_index = 0;
            for (auto const& run : m_tiers[new_tier_no].m_runs)
            {
              if (!run.active)
              {
                break;
            ...
            run& final = m_tiers[new_tier_no].m_runs[first_inactive_index];
            final.active = true;
            final.m_run_size = m_in_memory_table_max_size * pow(RunsPerTier, new_tier_no);

            auto a = get_vector_size_for_n_tiers(m_tiers[new_tier_no].m_level);
            if (m_elements.size() < get_vector_size_for_n_tiers(m_tiers[new_tier_no].m_level))
            {
              m_elements.resize(get_vector_size_for_n_tiers(m_tiers[new_tier_no].m_level));
            }

            job j;
            j.tier_source = tier_no;
            j.tier_dest = new_tier_no;
            j.first_inactive_index = first_inactive_index;
        ...
        if (!jobs.empty())
        {
          ...
          auto t = std::thread([this, jobs, &ready, &ready_mtx, &ready_cv]()
          {
            ++m_mergings;
            ...
            {
              auto tier_no = j.tier_source;
              auto new_tier_no = j.tier_dest;
              auto first_inactive_index = j.first_inactive_index;

              std::mutex& m1 = *(m_tiers[tier_no].m_merging);
              std::mutex& m2 = *(m_tiers[new_tier_no].m_merging);
              ...
              merge_runs(m_tiers[tier_no], m_tiers[new_tier_no], first_inactive_index);
      ...
      void merge_runs(tier const& source_tier, tier& dest_tier, int first_inactive_index)
      {
        ...
        if (dest_tier.m_run.size() < get_vector_size_for_tier(dest_tier.m_level))
        {
          dest_tier.m_run.resize(get_vector_size_for_tier(dest_tier.m_level));
        }
        // auto a = get_vector_size_for_n_tiers(dest_tier.m_level);
        ...
        for (int i = 0; i < RunsPerTier; ++i)
        {
          ...
          auto const& v = source_tier.m_run;
