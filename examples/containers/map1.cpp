...
// #define DATA_NODE_BLOCK_SIZE (256 * 1024)
// #define DATA_LEAF_BLOCK_SIZE (4 * 1024)
#define DATA_LEAF_BLOCK_SIZE (256 * 1024)
...
//     map_type my_map(DATA_NODE_BLOCK_SIZE * 3, DATA_LEAF_BLOCK_SIZE * 3);
//     // map_type my_map(64*MiB, 64*MiB);
//
...
//     for (auto d : data)
//         value_array.push_back(std::make_pair(static_cast<int>(d), (char)'a'));
//
...
//     }
//
//     // my_map.insert(value_array.begin(), value_array.end());
//
...
#define IO_DETAILS
  ...
  const int node_cache = DATA_NODE_BLOCK_SIZE * 60;
  const int leaf_cache = DATA_LEAF_BLOCK_SIZE * 60;
  const int node_block_size = DATA_NODE_BLOCK_SIZE;
  const int leaf_block_size = DATA_LEAF_BLOCK_SIZE;

  const std::string filename = get_available_filename("stxxl_map.txt");
